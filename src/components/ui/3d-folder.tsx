...
export function AnimatedFolder({ title, projects, description, className }: AnimatedFolderProps) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Emerging description on hover */}
        {description && (
          <p
            className="text-xs text-muted-foreground text-center mt-3 max-w-[240px] px-2 transition-all duration-500 ease-out"
            style={{
        ...
        >
          <span>Coloca el cursor para explorar</span>
        </div>
      </div>
...
export function ImageLightbox({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (wasOpen && currentIndex !== internalIndex) {
        setIsSliding(true)
        setSlideDirection(currentIndex > internalIndex ? "right" : "left")
  ...
  }, [currentIndex, isOpen, internalIndex])
  ...
  const getInitialStyles = (): React.CSSProperties => {
    if (!sourceRect || typeof window === "undefined") return {}
                ...
                <div className="flex items-center gap-3 mt-3 select-none">
                  <div className="flex items-center text-xs text-muted-foreground">
                    <button
                      ...
                      disabled={!hasPrev || isSliding}
                      className="px-1.5 py-0.5 mx-0.5 text-[10px] font-medium bg-muted hover:bg-muted/80 text-muted-foreground hover:text-foreground rounded border border-border transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
